// ═══════════════════════════════════════════════════════════════
//  PROJECT CARD — Premium Black & Gold (Modal-Click Support)
// ═══════════════════════════════════════════════════════════════
...
export default function ProjectCard({ project, index = 0, onClick }) {
  ...
  const handleCardClick = (e) => {
    // Don't open modal if they clicked a link
    if (e.target.closest("a")) return;
    // Call onClick if provided (for modal), otherwise do nothing
    if (onClick) onClick();
  };
        ...
        {techStack.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-5">
            {techStack.slice(0, 4).map((t) => (
              <span
                key={t}
                className="text-xs font-mono text-[#F0D060] bg-[#D4AF37]/[0.08] px-2.5 py-1 rounded-full border border-[#D4AF37]/15"
              >
                {t}
              </span>
            ))}
