// ═══════════════════════════════════════════════════════════════
//  SKILLS — Premium Black & Gold (9-Grid + Popup Modal)
// ═══════════════════════════════════════════════════════════════
...
const cardVariants = { hidden: { y: 40, opacity: 0, scale: 0.94 }, visible: { y: 0, opacity: 1, scale: 1, transition: { type: "spring", stiffness: 110, damping: 14 } } };

const INITIAL_SKILL_COUNT = 9; // Show exactly 9 skills
...
const Skills = () => {
  ...
  useEffect(() => {
    const fetchSkills = async () => {
      ...
      } catch (error) { console.error("Failed to fetch skills:", error); setSkills([]); }
  ...
  const renderCard = (skill, index) => {
    ...
    const levelStr = skill.level || skill.proficiency ? `${skill.level || skill.proficiency}%` : "0%";
    ...
    return (
      ...
      >
        ...
        <div className="relative z-10 text-center">
          <h3 className="font-display text-sm sm:text-base font-semibold text-[#FFFFFF] mb-3 transition-colors duration-300 group-hover:text-[var(--accent)]">{skill.name}</h3>
  ...
  return (
    <>
      <section id="skills" className="relative min-h-screen w-full overflow-hidden bg-[#0A0A0A] text-[#FFFFFF] flex items-center justify-center py-24 px-4 font-[Inter]">
        <style>{`
        ...
        <div className="relative z-10 container mx-auto max-w-5xl">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
            ...
            <h2 className="font-display font-bold tracking-tight text-4xl sm:text-5xl lg:text-6xl text-[#FFFFFF]">
              Technical{" "}
              <span className="bg-gradient-to-r from-[#D4AF37] via-[#E8C847] to-[#F0D060] bg-clip-text text-transparent">Skill Set</span>
            </h2>
            <p className="mt-4 text-[#A3A3A3] text-base sm:text-lg max-w-2xl mx-auto">Technologies and tools I use to bring products to life.</p>
          </motion.div>
          ...
          {loading ? (
            ...
            <p className="text-center text-[#A3A3A3] text-lg py-24">No skills added yet.</p>
          ) : (
            <>
              {/* 3x3 Grid (Exactly 9 Skills) */}
              <motion.div className="grid grid-cols-2 sm:grid-cols-3 gap-5 sm:gap-6" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
